...
def render_graph_ReSTIR_FPDG_NRD():
    g = RenderGraph('ReSTIR_FPDG_NRD')
    ...
    g.create_pass('ReSTIR_FPDG', 'ReSTIR_FPDG', {'PhotonBufferSizeGlobal': 400000, 'PhotonBufferSizeCaustic': 400000, 'AnalyticEmissiveRatio': 0.3499999940395355, 'PhotonBouncesGlobal': 10, 'PhotonBouncesCaustic': 10, 'PhotonRadiusGlobal': 0.019999999552965164, 'PhotonRadiusCaustic': 0.00800000037997961, 'EnableStochCollect': True, 'StochCollectK': 3, 'EnablePhotonCullingGlobal': False, 'EnablePhotonCullingCaustic': False, 'CullingRadius': 0.10000000149011612, 'CullingBits': 20, 'CausticCollectionMode': 3, 'CausticResamplingMode': 2, 'EnableDynamicDispatch': True, 'NumDispatchedPhotons': 822784})
    ...
    g.add_edge('ReSTIR_FPDG.residualRadiance', 'ModulateIllumination.residualRadiance')
    g.add_edge('AccumulatePass.output', 'ToneMapper.src')
    g.add_edge('NRD.filteredSpecularRadianceHitDist', 'ModulateIllumination.specularRadiance')
    g.add_edge('ReSTIR_FPDG.emission', 'ModulateIllumination.emission')
    g.add_edge('ReSTIR_FPDG.diffuseReflectance', 'ModulateIllumination.diffuseReflectance')
    g.add_edge('ReSTIR_FPDG.specularReflectance', 'ModulateIllumination.specularReflectance')
    g.add_edge('NRD.filteredDiffuseRadianceHitDist', 'ModulateIllumination.diffuseRadiance')
    g.add_edge('ReSTIR_FPDG.diffuseRadiance', 'NRD.diffuseRadianceHitDist')
    g.add_edge('ReSTIR_FPDG.specularRadiance', 'NRD.specularRadianceHitDist')
    ...
    g.add_edge('GBufferRT.vbuffer', 'ReSTIR_FPDG.vbuffer')
    g.add_edge('GBufferRT.mvec', 'ReSTIR_FPDG.mvec')
    g.add_edge('NRD.outValidation', 'ModulateIllumination.debugLayer')
    g.mark_output('ToneMapper.dst')
    g.mark_output('AccumulatePass.output')
    return g
m.addGraph(render_graph_ReSTIR_FPDG_NRD())
