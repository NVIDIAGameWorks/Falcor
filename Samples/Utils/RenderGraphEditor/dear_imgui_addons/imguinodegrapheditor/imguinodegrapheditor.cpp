...
#   endif //DEBUG_LINK_CULLING
            ...
            else if (isSomeNodeMoving) ImGui::SetMouseCursor(ImGuiMouseCursor_ResizeEW);
...
template<typename T> inline static T GetRadiansToDegs() {
    static T factor = T(180)/(3.1415926535897932384626433832795029f);
    return factor;
}
...
bool FieldInfo::render(int nodeWidth)   {
    ...
    if (f.type!=FT_UNSIGNED && f.type!=FT_INT)  {
        if (f.precision>0) {
            ...
            precisionLastCharIndex = static_cast<int>(strlen(precisionStr)) - 1;
        }
        else {
    ...
    //    const ImGuiDataType dataType = ImGuiDataType_Double;
    ...
    //        if (f.numArrayElements>1) changed = ImGui::Drag(label,value,f.numArrayElements,dragSpeed,minValue,maxValue,precisionStr);
    ...
    case FT_INT: {
        ...
        if (NodeGraphEditor::UseSlidersInsteadOfDragControls)   {
            if (f.numArrayElements>1) changed = ImGui::SliderIntN(label,value,f.numArrayElements,minValue,maxValue,precisionStr);
            else changed = ImGui::SliderInt(label,value,minValue,maxValue,precisionStr);
        }
        else {
            if (dragSpeed<1.f) dragSpeed = 1.f;
            if (f.numArrayElements>1) changed = ImGui::DragIntN(label,value,f.numArrayElements,dragSpeed,minValue,maxValue,precisionStr);
            else changed = ImGui::DragInt(label,value,dragSpeed,minValue,maxValue,precisionStr);
        }
