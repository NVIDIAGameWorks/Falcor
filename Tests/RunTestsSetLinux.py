...
machine_default_checkin_reference_directory = '/home/nvrgfx/Desktop/ReferenceResults/'

class TestsSetError(Exception):
    pass

# Get the Executable Directory.
def get_executable_directory(testSet):
    return testSet + "/Bin/"

# Build the Solution.
def build_solution(clonedDir, configuration, rebuild):
    #print("Please build the solution through the Makefile manually")
    prevDir = os.getcwd()
    os.chdir(clonedDir)
...
def run_test_run(executable_filepath, current_arguments, output_file_base_name, output_directory):
    ...
    try:
        ...
        while process.returncode is None:
            ...
            if difference_time > machine_process_default_kill_time:
                print("Kill Process")
                process.kill()
...
def run_tests_set(main_directory, rebuild, json_filepath, results_directory, reference_directory):
    ...
    try:
        ...
        build_solution(main_directory, tests_set_run_data['Configuration Target'], rebuild)
    ...
    for current_tests_group_name in tests_set_run_data['Tests Groups']:
        ...
        if current_tests_group['Enabled'] == True:
            ...
            # Get the executable directory.
            executable_directory = absolutepath + '/' + get_executable_directory(tests_set_run_data['Name'])
            ...
            if not os.path.isdir(current_results_directory):
                os.makedirs(current_results_directory)
            else:
                shutil.rmtree(current_results_directory)
                os.makedirs(current_results_directory)
...
def analyze_tests_group(tests_set_data, current_test_group_name):
    ...
    for index, current_test_args in enumerate(current_test_group['Project Tests Args']):
        if index in current_test_group['Results']['Errors']:
            ...
            current_test_reference_directory = str(tests_set_data['Reference Directory'] + '/' + current_test_group_name + '/')
...
def analyze_screen_captures(tolerance, result_json_data, current_test_result_directory, current_test_reference_directory):
    ...
    for key in ['Frame Screen Captures', 'Time Screen Captures']:
        ...
        for index, frame_screen_captures in enumerate(result_json_data[key]):
            ...
            # Run ImageMagick
            image_compare_command = ['compare', '-metric', 'MSE', '-compose', 'Src', '-highlight-color', 'White', '-lowlight-color', 'Black', test_result_image_filename, test_reference_image_filename, test_compare_image_filepath]
...
def main():
    ...
    main_results_directory = machine_relative_checkin_local_results_directory
    main_reference_directory = machine_default_checkin_reference_directory
    ...
    html_file_path = os.path.join(machine_relative_checkin_local_results_directory, helpers.build_html_filename(tests_set_data))
    html_file = open(html_file_path, 'w')
    html_file.write(html_file_content)
    html_file.close()

    # Open it up.
    os.system("firefox " + html_file_path)
